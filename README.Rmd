...
Alternatively, one can also perform a score test variance component using `taps_score_test` ([Zhang and Lin, 2003](https://doi.org/10.1093/biostatistics/4.1.57)). We use `CompQuadForm::farebrother` as the default method to compute the p-value.
...
```
This is evidence that median GAM can be used to make the fit more robust. \textbf{Currently, score tests have been extended to partial exponential family models, including `cox.ph`, `ocat`, `nb`, `tw`, `betar`, `scat`, and `qgam`.} The results are as follows:
