...
# Alternative seeds: 520001--520100
for (i in seq_len(nsim)) {
  set.seed(520000L + i)
  X <- MASS::mvrnorm(n, rep(0, 4), matrix(0.25, 4, 4) + 0.75 * diag(4))
  x0 <- qbeta(pnorm(X[, 1]), 1.5, 1.5)
...
if (!identical(lengths(out), c(null = 100L, alternative = 100L)) ||
  ...
  stop("scat simulation must produce 100 finite p-values per scenario.")
}
saveRDS(out, file.path("example", "scat_pvalues.rds"))
