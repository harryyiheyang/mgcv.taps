...
for (i in seq_len(nsim)) {
  ...
  eta <- f1 + f2 + f3
  y <- rnbinom(n, mu = exp(eta / 2 - 1), size = 1.5)
...
# Alternative seeds: 420001--420100
for (i in seq_len(nsim)) {
  set.seed(420000L + i)
  X <- MASS::mvrnorm(n, rep(0, 4), matrix(0.25, 4, 4) + 0.75 * diag(4))
  x0 <- qbeta(pnorm(X[, 1]), 1.5, 1.5)
...
if (!identical(lengths(out), c(null = 100L, alternative = 100L)) ||
  ...
  stop("NB simulation must produce 100 finite p-values per scenario.")
}
saveRDS(out, file.path("example", "nb_pvalues.rds"))
