library(mgcv)
devtools::load_all(".")

n <- 1000L
nsim <- 100L
pv_null <- numeric(nsim)
pv_alt <- numeric(nsim)

# NULL seeds: 210001--210100
for (i in seq_len(nsim)) {
  set.seed(210000L + i)
  X <- MASS::mvrnorm(n, rep(0, 4), matrix(0.25, 4, 4) + 0.75 * diag(4))
  x1 <- qbeta(pnorm(X[, 1]), 1.5, 1.5)
  x2 <- qbeta(pnorm(X[, 2]), 1.5, 1.5)
  x3 <- qbeta(pnorm(X[, 3]), 1.5, 1.5)
  x4 <- qbeta(pnorm(X[, 4]), 1.5, 1.5)
  X1 <- linearity_discontinuity(x1, 0.5)[, -1]
  f1 <- as.numeric(X1 %*% c(4, 4, 0))
  t2 <- 2 * pi * x2
  f2 <- 0.4 * sin(t2) + 0.8 * cos(t2) + 1.2 * sin(t2)^2 +
    1.6 * cos(t2)^3 + 2 * sin(t2)^3
  t3 <- 2 * (x3 - 0.5)
  f3 <- 3 * sin(3 * t3) + 6 * exp(-36 * t3^2)
  y <- rbinom(n, 1, pnorm((f1 + f2 + f3) / 2 - 5 / 2))
  dat <- data.frame(x1, x2, x3, x4, y)
  b <- gam(
    y ~ s(x1, bs = "AMatern", k = 10, m = 100,
          xt = list(getA = linearity_discontinuity, para = 0.5)) +
      s(x2, bs = "cr", k = 10) + s(x3, bs = "cr", k = 15) +
      s(x4, bs = "cr", k = 10),
    data = dat, family = binomial(link = "probit"), method = "REML"
  )
  pv_null[i] <- taps_score_test(
    b, test.component = 1, method = "davies"
  )$smooth.pvalue
}

# Alternative seeds: 220001--220100
for (i in seq_len(nsim)) {
  set.seed(220000L + i)
  X <- MASS::mvrnorm(n, rep(0, 4), matrix(0.25, 4, 4) + 0.75 * diag(4))
  x1 <- qbeta(pnorm(X[, 1]), 1.5, 1.5)
  x2 <- qbeta(pnorm(X[, 2]), 1.5, 1.5)
  x3 <- qbeta(pnorm(X[, 3]), 1.5, 1.5)
  x4 <- qbeta(pnorm(X[, 4]), 1.5, 1.5)
  X1 <- smoothed_linearity_discontinuity(x1, list(0.5, 0.05))[, -1]
  f1 <- as.numeric(X1 %*% c(4, 4, 0))
  t2 <- 2 * pi * x2
  f2 <- 0.4 * sin(t2) + 0.8 * cos(t2) + 1.2 * sin(t2)^2 +
    1.6 * cos(t2)^3 + 2 * sin(t2)^3
  t3 <- 2 * (x3 - 0.5)
  f3 <- 3 * sin(3 * t3) + 6 * exp(-36 * t3^2)
  y <- rbinom(n, 1, pnorm((f1 + f2 + f3) / 2 - 5 / 2))
  dat <- data.frame(x1, x2, x3, x4, y)
  b <- gam(
    y ~ s(x1, bs = "AMatern", k = 10, m = 100,
          xt = list(getA = linearity_discontinuity, para = 0.5)) +
      s(x2, bs = "cr", k = 10) + s(x3, bs = "cr", k = 15) +
      s(x4, bs = "cr", k = 10),
    data = dat, family = binomial(link = "probit"), method = "REML"
  )
  pv_alt[i] <- taps_score_test(
    b, test.component = 1, method = "davies"
  )$smooth.pvalue
}

out <- list(null = pv_null, alternative = pv_alt)
if (!identical(lengths(out), c(null = 100L, alternative = 100L)) ||
    any(!is.finite(unlist(out, use.names = FALSE)))) {
  stop("Binary simulation must produce 100 finite p-values per scenario.")
}
saveRDS(out, file.path("example", "binary_pvalues.rds"))
