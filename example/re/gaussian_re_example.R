...
for (i in seq_len(nsim)) {
  ...
  y <- 1 + f1 + f2 + f3 + u[as.integer(cl)] + rnorm(n)
...
# Alternative seeds: 5120001--5120100
for (i in seq_len(nsim)) {
  set.seed(5120000L + i)
  cl <- factor(rep(seq_len(ncl), each = mcl))
  x0 <- qbeta(pnorm(rnorm(ncl)), 1.5, 1.5)[as.integer(cl)]
...
if (any(!is.finite(unlist(out, use.names = FALSE)))) {
  stop("Gaussian RE simulation produced non-finite p-values.")
}
saveRDS(out, file.path("example", "re", "gaussian_re_pvalues.rds"))
