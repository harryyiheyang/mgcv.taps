...
for (i in seq_len(nsim)) {
  ...
  mu <- binomial()$linkinv((f1 + f2 + f3) / 2 - 2 + u[as.integer(cl)])
...
# Alternative seeds: 5620001--5620100
for (i in seq_len(nsim)) {
  set.seed(5620000L + i)
  cl <- factor(rep(seq_len(ncl), each = mcl))
  x0 <- qbeta(pnorm(rnorm(ncl)), 1.5, 1.5)[as.integer(cl)]
...
if (any(!is.finite(unlist(out, use.names = FALSE)))) {
  stop("sbeta RE simulation produced non-finite p-values.")
}
saveRDS(out, file.path("example", "re", "sbeta_re_pvalues.rds"))
