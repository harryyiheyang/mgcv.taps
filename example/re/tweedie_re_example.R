...
for (i in seq_len(nsim)) {
  ...
  eta <- f1 + f2 + f3
  y <- rtweedie(n, mu = exp(eta / 2 - 1 + u[as.integer(cl)]),
                phi = 3, power = 1.5)
...
# Alternative seeds: 5720001--5720100
for (i in seq_len(nsim)) {
  set.seed(5720000L + i)
  cl <- factor(rep(seq_len(ncl), each = mcl))
  x0 <- qbeta(pnorm(rnorm(ncl)), 1.5, 1.5)[as.integer(cl)]
...
if (any(!is.finite(unlist(out, use.names = FALSE)))) {
  stop("Tweedie RE simulation produced non-finite p-values.")
}
saveRDS(out, file.path("example", "re", "tweedie_re_pvalues.rds"))
