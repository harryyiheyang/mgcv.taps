...
for (i in seq_len(nsim)) {
  ...
  eta <- f1 + f2 + f3
  y <- rnbinom(n, mu = exp(eta / 2 - 1 + u[as.integer(cl)]), size = 1.5)
...
# Alternative seeds: 5420001--5420100
for (i in seq_len(nsim)) {
  set.seed(5420000L + i)
  cl <- factor(rep(seq_len(ncl), each = mcl))
  x0 <- qbeta(pnorm(rnorm(ncl)), 1.5, 1.5)[as.integer(cl)]
...
if (any(!is.finite(unlist(out, use.names = FALSE)))) {
  stop("NB RE simulation produced non-finite p-values.")
}
saveRDS(out, file.path("example", "re", "nb_re_pvalues.rds"))
